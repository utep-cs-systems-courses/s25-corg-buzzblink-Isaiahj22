...
int tune[] = {262, 294, 330, 349, 392, 440, 494, 523};
int currentNote = 0;
...
void state_advance() {
  ...
  if (isPlaying && currentNote < sizeof(tune) / sizeof(tune[0])) {
    switch (color) {
...
void startNote() {
  if (isPlaying && currentNote < sizeof(tune) / sizeof(tune[0])) {
    buzzer_set_period(tune[currentNote] == 0 ? 0 : 2000000 / tune[currentNote]);
    currentNote++;
  } else {
    currentNote = 0;
    buzzer_set_period(0);
